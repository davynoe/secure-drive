...
import { listFileMetadata, listAllSyncConnections, listSyncConnections, replaceFileMetadataForConnection, upsertFileMetadata, upsertSyncConnection, } from './syncStore';
...
ipcMain.handle('secure-drive:list-folder', async (_event, folderPath) => {
...
ipcMain.handle('secure-drive:list-sync-connections', async (_event, ownerUserId) => {
    if (typeof ownerUserId !== 'number' || Number.isNaN(ownerUserId)) {
        return [];
    }
    return listSyncConnections(ownerUserId);
});
ipcMain.handle('secure-drive:upsert-sync-connection', async (_event, input) => {
...
ipcMain.handle('secure-drive:list-file-metadata', async (_event, connectionId) => {
    if (typeof connectionId !== 'number' || Number.isNaN(connectionId)) {
        return [];
    }
    return listFileMetadata(connectionId);
});
ipcMain.handle('secure-drive:upsert-file-metadata', async (_event, connectionId, input) => {
...
ipcMain.handle('secure-drive:replace-file-metadata', async (_event, connectionId, files) => {
    if (typeof connectionId !== 'number' || Number.isNaN(connectionId) || !Array.isArray(files)) {
        return [];
    }
    return replaceFileMetadataForConnection(connectionId, files.map((file) => ({
